...
import {youtubePreview, mountYouTube, youtubeSelection} from './youtube.js';
...
export async function openClip(id){
 ...
 try{
  ...
  showDialog(dialogHeader('CLIP DESK',clip.title)+`<form id="clip-form"><div class="drawer-body"><div class="clip-stage"><span class="pill ${clip.approved?'success':''}">${esc(clip.status==='ready'?'Ready to review':clip.status)}</span><span class="small muted">Revision ${clip.revision} · ${clip.has_render?'Rendered video available':'Original source preview'}</span></div>${preview(clip)}<div class="drawer-meta"><a data-episode-source href="${safeURL(clip.source_link)}" target="_blank" rel="noopener noreferrer">${esc(clip.episode_title)} ↗</a></div>${clip.error?errorPanel(clip.error):''}<label>Clip title<input name="title" value="${esc(clip.title)}" required maxlength="250"></label><div class="trim-row"><label>Start time<input name="start" value="${time(clip.start)}" placeholder="0:00" required aria-describedby="time-help"></label><span class="trim-dash">—</span><label>End time<input name="end" value="${time(clip.end)}" placeholder="1:00" required></label><span class="duration-display" id="clip-duration">${Math.round(clip.end-clip.start)}s</span></div><p id="time-help" class="small muted">Use mm:ss or hh:mm:ss. Up to 180 seconds.</p><fieldset class="format-choices"><legend>Format</legend>${[['9:16','Portrait'],['1:1','Square'],['16:9','Landscape']].map(([v,label])=>`<label class="format-choice"><input type="radio" name="format" value="${v}" ${clip.format===v?'checked':''}><span class="format-symbol ${label.toLowerCase()}"></span><span>${label}<small>${v}</small></span></label>`).join('')}</fieldset><p class="small muted">Video is center-cropped to the selected format.</p><label>Post caption<textarea name="caption" rows="4" maxlength="5000">${esc(clip.caption)}</textarea></label><details class="source-passage"><summary>Read the saved passage</summary><p>${esc(clip.transcript_text)}</p><span class="small muted">The saved source text is unchanged by trim edits.</span></details><div class="media-section"><div class="section-heading"><h3>Source video</h3><span class="small muted">Connected in the full Studio</span></div><div class="notice">${icon('link')}<span><strong>Your own video library, connected.</strong><br>The full Studio connects directly to the original episode files on your computer or network. It finds the matching video, cuts and formats each clip automatically, and after your review and approval exports it to your social channels. No uploads and no YouTube downloads.</span></div></div>${clip.delivery?`<div class="notice"><span>Publisher delivery: <strong>${esc(clip.delivery.status)}</strong><br>${esc(clip.delivery.receipt||'Waiting for the connected workflow')}</span></div>`:''}<div id="clip-message" role="status"></div></div><div class="drawer-actions clip-actions"><button class="btn secondary" type="submit" id="save-clip">Save changes</button><button class="btn primary" type="button" id="render-clip" ${clip.asset_id&&clip.status!=='rendering'?'':'disabled'} title="${clip.asset_id?'':'Rendering runs against your connected video library in the full Studio'}">${icon('clips')} ${clip.status==='rendering'?'Rendering…':'Render preview'}</button><div class="secondary-actions"><a href="/api/clips/${id}/export" class="text-btn" id="export-clip">${icon('download')} ${clip.has_render?'Export clip package':'Export editorial notes'}</a><button class="text-btn" type="button" id="approve-clip" ${!clip.has_render||clip.approved?'disabled':''}>${icon('check')} ${clip.approved?'Approved':'Approve cut'}</button>${connections.publisher?`<button class="text-btn" type="button" id="publish-clip" ${!clip.approved||clip.delivery?'disabled':''}>Send to publisher ${icon('arrow')}</button>`:''}</div></div></form>`);
  const youtube=wirePreview(clip);
  const form=$('#clip-form');
  form.oninput=()=>{
   dirty=true;$('#approve-clip').disabled=true;if($('#publish-clip'))$('#publish-clip').disabled=true;
   const start=form.elements.start.value.trim()?parseTime(form.elements.start.value):NaN;
   const end=form.elements.end.value.trim()?parseTime(form.elements.end.value):NaN;
   const duration=end-start;$('#clip-duration').textContent=Number.isFinite(duration)?`${Math.round(duration)}s`:'—';
   youtube?.update(start,end);
   if(clip.youtube_id){const source=$('[data-episode-source]');try{source.href=youtubeSelection(clip.youtube_id,start,end,location.origin).watch;source.hidden=false;}catch{source.hidden=true;}}
   $('#clip-message').textContent='Unsaved changes';
  };
  async function save(){
   ...
   clip=await api(`/clips/${id}`,{method:'PATCH',body:{title:values.title,caption:values.caption,start,end,format:values.format,asset_id:clip.asset_id||null,alignment_confirmed:!!clip.alignment_confirmed,revision:clip.revision}});
